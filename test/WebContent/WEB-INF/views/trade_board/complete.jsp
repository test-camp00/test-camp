	...
	<div class="well well-lg">
		...
		<p style="max-width: 400px; margin: 0 auto;">
			...
			<button type="button" class="btn btn-primary btn-lg btn-block"
				onclick="javascript:location.href='<%=cp%>/trade_board/board_trade.do';">게시글 리스트로 이동
				&raquo;</button>
			</c:if>	
			<c:if test="${mode2=='update'}">
			<button type="button" class="btn btn-primary btn-lg btn-block"
				onclick="javascript:location.href='<%=cp%>/trade_board/board_trade.do?page=${page }';">게시글 리스트로 이동
				&raquo;</button>
			</c:if>		
		</p>
	</div>
